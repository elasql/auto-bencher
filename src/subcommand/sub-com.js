...
function generate_connection_list(config, parameter, action){
    ...
    sequencer = null;
    if('sequencer' in config.machines)
        sequencer = new con_com.ConnectionInfo(server_count, config.machines.sequencer, 35000, 'sequencer');
...
function kill_benchmarker(config, machine){
    ...
    if(!bool)
        console.log("No existing process is found on " + machine.ip);
}