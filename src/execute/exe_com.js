...
module.exports = {
    execute: async function(config, db_name, action, report_dir,vm_args, sequencer, server_list, client_list){
        ...
        machines_func = await machines.map(conn_info => {
            if(conn_info.prop == 'sequencer')
                return new server.async_server_exec(config, conn_info, db_name, seq_vm_args, true, action);
            else if(conn_info.prop == 'server')
                return new server.async_server_exec(config, conn_info, db_name, server_vm_args, false, action);
            else if(conn_info.prop == 'client')
                return new client.async_client_exec(config, conn_info, client_vm_args, action);
        });
